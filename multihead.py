...
class MultiHeadAttention(nn.Module):  # 定义多头注意力类，继承 nn.Module
    def __init__(self, embed_size, num_heads):  # 构造函数，初始化嵌入维度和头数
        ...
        assert embed_size % num_heads == 0, "Embedding size must be divisible by num_heads"  # 确保嵌入维度能被头数整除

        self.num_heads = num_heads  # 记录头数
        self.head_dim = embed_size // num_heads  # 每个头的维度，嵌入维度除以头数

        # 初始化线性变换层，将输入映射到查询、键、值空间
        self.query_linear = nn.Linear(embed_size, embed_size)  
        self.key_linear = nn.Linear(embed_size, embed_size)
        self.value_linear = nn.Linear(embed_size, embed_size)

        # 最后的线性变换，将所有头的输出合并到一起
    ...
    def forward(self, query, key, value, mask=None):  # 前向传播，计算注意力
        ...
        # 如果有mask，屏蔽某些位置
        if mask is not None:
            attention_scores = attention_scores.masked_fill(mask == 0, float('-inf'))  # 将mask对应位置的得分设置为负无穷
