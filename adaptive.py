...
class AdaptiveAttention(nn.Module):  # 定义自适应注意力类，继承 nn.Module
    def __init__(self, embed_size, num_heads, alpha=1.0):  # 构造函数，初始化嵌入维度、头数以及 alpha
        super(AdaptiveAttention, self).__init__()  # 调用父类构造函数
        self.num_heads = num_heads  # 记录头数
        self.head_dim = embed_size // num_heads  # 每个头的维度
        self.alpha = alpha  # 自适应调整的缩放因子
    ...
    def forward(self, query, key, value, mask=None):  # 前向传播，计算注意力
        ...
        # 自适应调整 attention scores 的缩放因子
        attention_scores *= self.alpha  # 注意力得分乘以 alpha 进行缩放
